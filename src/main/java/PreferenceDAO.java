...
public class PreferenceDAO {
	...
	public static void updatePreference(preferencesService e) {
	...
	public static preferencesService getPreference(int uid) {
		preferencesService p=null;
		try(Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
		         Statement stmt = conn.createStatement();
			 ...
			 while (rs.next()) {
				// Retrieve by column name
				p=new preferencesService(uid,rs.getBoolean("notification"),rs.getBoolean("private"),rs.getString("gender"));
			 }
		  } catch (SQLException ex) {
			 ex.printStackTrace();
		  }
